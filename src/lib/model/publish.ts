 ...
 */
export function abandonWithMeeting(races: PublishedRace[], calledOff = new Set<string>()): PublishedRace[] {
  ...
  const kept = races.map((r) => (!r.abandoned && !r.result?.length && calledOff.has(r.raceId) ? abandonRace(r) : r));
