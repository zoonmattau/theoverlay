...
import { hubDistances, listedDistance } from "@/lib/data/hub";
...
async function Distances({ searchParams }: { searchParams: PageProps<"/data/distances">["searchParams"] }) {
  ...
  const distances = (await hubDistances()).filter(listedDistance);
  return (
    <section className="py-6">
      <p className="mb-3 text-sm text-ink-secondary">The typical time over each distance run at two tracks or more, the track that runs it quickest, and the record we hold. Click a distance for its page: every track that runs it, where the leaders win over it, who rides and trains well over the trip, and what is on over it next.</p>
