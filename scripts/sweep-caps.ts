...
const races = readdirSync(".formking-cache")
  ...
  // =all keeps every day. The default is the clean days: the form behind the August Saturdays and 5 Sep was fetched after
  // those races, and fields beyond the run list (days since the last run, the feed's ratings, the form strings) saw later
  // starts, so on those days they carry the opposite sign to the clean ones (scripts/out/leak-coef.ts, 20 Sep 2026).
  .filter((r) => {
    const want = process.env.OVERLAY_SWEEP_DAYS ?? "from:2026-09-11";
